...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        const data = (await res.json()) as { user: User; posts: Post[] };
        setUser(data.user);
        setPosts(data.posts);
      } finally {
        setLoading(false);
  ...
  async function showFollowerStats(kind: "followers" | "following") {
    const res = await fetch(`/api/profile/${username}/${kind}`);
    if (!res.ok) return;
    const data = (await res.json()) as { count: number };
    window.alert(`${data.count.toLocaleString()} ${kind}`);
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-6 mb-4">
            ...
            <button
              ...
              onClick={() => showFollowerStats("followers")}
            >
              <span className="font-semibold">
            ...
            <button
              className="hover:opacity-70"
              onClick={() => showFollowerStats("following")}
            >
              <span className="font-semibold">
