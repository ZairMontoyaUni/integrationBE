...
export default function MessageThreadPage() {
  ...
  useEffect(() => {
    async function loadThreadData() {
      try {
        const [conversationRes, conversationsRes] = await Promise.all([
          fetch(`/api/messages/${id}`),
          fetch("/api/messages"),
        ]);

        if (conversationRes.ok) {
          const conv = (await conversationRes.json()) as Conversation;
          setConversation(conv);
        }

        if (conversationsRes.ok) {
          const list = (await conversationsRes.json()) as Conversation[];
